...
#$SheetsName = @("ЦОД") # эту строчку можно раскомментить, если необходимо чекнуть все блейд шасси только в одном листе, указав какой именно лист нужен 
$deshdesh = "https://"
            ...
            if ($CellsValue.Contains($deshdesh)){
                ...
                for ($i = 0; $i -lt 16; $i++){
                    if ($Shassi.ServerBlade.ServerName[$i] -ne $null){
                        ...
                        $cpu1 = $Shassi.ServerBlade.cpu.value[$j]; $cpu2 = $Shassi.ServerBlade.cpu.value[$j+1]
...
$Workbook.SaveAs("$PSScriptRoot\Blades(new).xlsx")
$Workbook.Close($true)
$ExcelObj.Quit()